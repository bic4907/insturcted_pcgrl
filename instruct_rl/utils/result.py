...
def calculate_progress(row: pd.Series, condition_col: str, feat_col: str) -> float:
    ...
    progress = (row[feat_col] - row[f's0_{feat_col}']) / (row[condition_col] - row[f's0_{feat_col}'] + 0.0000001)
...
def process_result(raw_df: pd.DataFrame, div_df: pd.DataFrame,
    ...
    df = df.groupby(['exp_id', 'exp_seed', 'instruct', 'row_i', 'reward_enum',
                     'cond_region', 'cond_plength', 'cond_nblock', 'cond_nbat', 'cond_batdir'
                                                   ...
                                                   'progress_nbat': 'mean',
                                                   'progress_batdir': 'mean'}).reset_index()
