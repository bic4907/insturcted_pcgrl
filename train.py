...
def make_train(config, restored_ckpt, checkpoint_manager, encoder_params):
    ...
    def train(rng, runner_state):
        ...
        # print(network.subnet.tabulate(_rng, init_x.map_obs, init_x.flat_obs, jnp.ones((init_x.nlp_obs.shape[0], 128)), jnp.ones((init_x.nlp_obs.shape[0], 128))))
        ...
        def _update_step_with_render(update_runner_state, _):
            ...
            def _evaluate_step():
                ...
                if test_inst is not None:
                    # random_indices = jax.random.randint(  
                    #     # Array([3, 1, 3, 1], dtype=int32)  
                    #     rng,
                    #     (config.n_envs,),
                    #     0,
                    #     test_inst.reward_i.shape[0],
                    # )
