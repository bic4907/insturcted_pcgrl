...
def extract_unique_pair_indices(pairs: np.ndarray) -> np.ndarray:
    """
    Returns the indices of unique pairs from a NumPy array.

    Parameters:
        pairs (np.ndarray): A 2D NumPy array of shape (n, 2), where each row represents a pair.

    Returns:
        np.ndarray: A 1D array containing the indices of unique pairs in the original array.
