...
class PCGRLEnv(Environment):
    ...
    def get_obs(self, env_map, frz_map, rep_state, prob_state):
        ...
        if self.nlp_input_dim > 0:
            ...
            obs = PCGRLObs(map_obs=rep_obs, flat_obs=prob_obs, nlp_obs=nlp_obs)
        elif self.vec_input_dim > 0:
            vec_obs = jnp.zeros(8)
            obs = PCGRLObs(map_obs=rep_obs, flat_obs=prob_obs, nlp_obs=vec_obs)
        else:
            obs = PCGRLObs(
                map_obs=rep_obs,
                flat_obs=prob_obs,
    ...
    def gen_dummy_obs(self, env_params: PCGRLEnvParams):
        ...
        if env_params.nlp_input_dim > 0:
            ...
            return PCGRLObs(map_x, ctrl_x, nlp_obs)
        elif env_params.vec_input_dim > 0:
            vec_obs = jnp.zeros((1, 8))
            return PCGRLObs(map_x, ctrl_x, vec_obs)
        else:
            return PCGRLObs(map_x, ctrl_x, jnp.zeros((1,)))
